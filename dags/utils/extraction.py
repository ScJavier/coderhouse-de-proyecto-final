...
def format_exchange_rates(api_response, base_currency, target_currencies, target_date):
    """
    ...
    - target_date (str): The date for which to retrieve exchange rates (format: 'YYYY-MM-DD').
    ...
    for currency in target_currencies:
        ...
        formatted_rates['date'].append(target_date)
        formatted_rates['currency'].append(currency)
        formatted_rates[rate_name].append(api_response['rates'][currency])
        
    return formatted_rates


def get_currencies_date(base_currency, target_date):
    """
    Get formatted exchange rates for a specific base currency and date.
    ...
    url = f"https://api.frankfurter.app/{target_date}?from={base_currency}&to={','.join(target_currencies)}"
    ...
    if response.status_code == 200:
        ...
        return format_exchange_rates(response.json(), base_currency, target_currencies, target_date)
    else:
        # If the API request fails, raise a ValueError with an error message
